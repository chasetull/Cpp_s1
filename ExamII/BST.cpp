...
void delOddLeafHelper(Node *startn)
{
    if (startn->parent != NULL)
    {
        cout << startn->key << endl;
    }
    
    if (startn->left == NULL && startn->right == NULL && startn->key % 2 != 0)
    {
        Node *prent = startn->parent;
        cout << "Delete: " << startn->key << endl;
        if (startn->parent != NULL && startn == startn->parent->right)
        {
            cout << startn->parent->key << endl;
        }
        if (prent->left == startn)
        {
            prent->left = NULL;
            free(startn);
        }
        else if (prent->right == startn)
        {
            prent->right = NULL;
            free(startn);
        }
    ...
    if (startn->left != NULL)
    {
        delOddLeafHelper(startn->left);
    }
    if (startn->right != NULL)
    {
        delOddLeafHelper(startn->right);
    }
